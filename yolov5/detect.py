...
from utils.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams, LoadPiCamera
...
class Detector:
    def __init__(self, source, weights, imgsz=640, half=False, save_img=False, view_img=False, dnn=False):
        source = str(source)
        self.source = source
        self.weights = weights
        self.save_img = save_img and not source.endswith('.txt')  # save inference images
        ...
        project = "output"
        name = "detections"
        self.save_dir = increment_path(Path(project) / name, exist_ok=True)  # increment run
        ...
        # Dataloader
        if source=="picamera":
            self.dataset = LoadPiCamera(img_size=self.imgsz, stride=self.stride)
            bs = len(self.dataset)  # batch_size
        elif self.webcam:
            self.view_img = check_imshow()
            dataset = LoadStreams(source, img_size=imgsz, stride=self.stride, auto=self.pt, vid_stride=1)
    ...
    def detect_once(self, conf_thres=0.25, iou_thres=0.45, grayscale=False):
        ...
        for i, det in enumerate(pred):  # per image
            seen += 1
            if self.webcam or self.source == "picamera":  # batch_size >= 1
                p, im0, frame = path, im0s.copy(), self.dataset.count
                s += f'{i}: '
            else:
